...
# Storage goal data from /eddie2_navigation/text/goal_solustar.txt
...
class systemAvailability(smach.State):
	...
	def execute(self, userdata):
		...
		localize_node = roslaunch.core.Node(package='eddie2sim_sensor', 
		...
		sound_node = roslaunch.core.Node(package='eddie2sim_sound', 
										 node_type='eddie2sim_soundplay.py', 
										 name='eddie2sim_soundplay_node')
...
class turn2goal(smach.State):
	...
	def execute(self, userdata):
		...
		turn2goal_node = roslaunch.core.Node(package='eddie2sim_sensor', 
											node_type='sim_rotateBy_odom.py', 
											name='sim_rotateBy_odom_node',
											output="screen")
...
class move2goal(smach.State):
	...
	def execute(self, userdata):
		...
		nav_node = roslaunch.core.Node(package='eddie2sim_navigation', 
...
class goalAlignment(smach.State):
	...
	def execute(self, userdata):
		...
		goalAlignment_node = roslaunch.core.Node(package='eddie2sim_sensor', 
													 node_type='sim_rotateBy_odom.py', 
													 name='sim_rotateBy_odom_node',
													 output="screen")
...
if __name__ == '__main__':
	# Define node name
	rospy.init_node('eddie2sim_fsm_node', anonymous=False)
	main()
